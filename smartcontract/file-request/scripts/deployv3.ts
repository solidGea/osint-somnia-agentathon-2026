...
async function main() {
  const baseUrl = process.env.OSINT_API_BASE_URL ?? 'https://privServer/api';
  ...
  console.log('Deploying FileRequestV3 to Somnia Testnet...\n');
  ...
  const fileRequest = await hre.viem.deployContract('FileRequestV3', [baseUrl, defaultSelector, feeRecipient]);

  console.log(`✅ FileRequestV3 deployed at: ${fileRequest.address}`);
  console.log('\nNext steps:');

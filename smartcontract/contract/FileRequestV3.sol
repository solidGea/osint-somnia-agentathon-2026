...
contract FileRequestV3 {
    ...
    function requestStringLookup(string calldata target, string calldata apiKey)
        external
        payable
        returns (uint256 requestId)
    {
        requestId = _requestStringData("string-lookup", target, defaultSelector, apiKey);
        requests[requestId].requester = msg.sender;
        requests[requestId].status = RequestStatus.Pending;
        emit RequestCreated(requestId, msg.sender, "string-lookup", target);
    }

    function requestData(string calldata queryType, string calldata target, string calldata apiKey)
        external
        payable
        returns (uint256 requestId)
    {
        requestId = _requestStringData(queryType, target, defaultSelector, apiKey);
        requests[requestId].requester = msg.sender;
        requests[requestId].status = RequestStatus.Pending;
    ...
    function requestDataWithSelector(
        ...
        requestId = _requestStringData(queryType, target, selector, apiKey);
        requests[requestId].requester = msg.sender;
        requests[requestId].status = RequestStatus.Pending;
        emit RequestCreated(requestId, msg.sender, queryType, target);
    }
    ...
    function _requestStringData(
        ...
        string memory url = string.concat(baseUrl, apiKey, "?type=", queryType, "&target=", target);
